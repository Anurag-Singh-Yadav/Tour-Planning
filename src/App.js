...
function App() {
  ...
  function removeCard(newId){
    // console.log('ansd');
      const newTour = newData.filter((e)=>{
          return (e.id !== newId);
      }) 
      setData(newTour);
  }
  return (
    <div className="App">
      <Tour></Tour>
      <div className="App flex flex-wrap justify-evenly px-[16px] gap-y-[30px]">
      {newData.map((obj, key) => (
        <Card key={key} image={obj.image} place={obj.name} price={obj.price} info={obj.info} remove={removeCard} id = {obj.id}/>
      ))}
      </div>
    </div>
  );
